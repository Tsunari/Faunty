...
class _ProgramOrganisationCalendarV2State extends ConsumerState<ProgramOrganisationCalendarV2> {
  ...
  DateTime _snap(DateTime dt) {
    final m = (dt.minute / _slotMinutes).round() * _slotMinutes;
  ...
  void _updateAt(Offset localPos) {
    ...
    final dy = localPos.dy + _scroll.offset;
    final slot = _dyToSlot(dy);
    final end = _slotToTime(slot + 1);
    setState(() {
      _tempBlock = _tempBlock!.copyWith(end: end);
    });
  }
  ...
  void _onBlockPanStart(_Block block, DragStartDetails details) {
  ...
  void _onBlockPanUpdate(DragUpdateDetails details) {
    if (_movingBlock == null || _movingBlockDuration == null) return;
    _movingAccumulatedDy += details.delta.dy;
    final slotShift = (_movingAccumulatedDy / _slotHeight).round();
    if (slotShift != 0) {
      final minutesShift = slotShift * _slotMinutes;
      final newStart = _movingBlock!.start.add(Duration(minutes: minutesShift));
      ...
      // consume applied pixels
      _movingAccumulatedDy -= slotShift * _slotHeight;
    }
  }

  void _onBlockPanEnd(DragEndDetails details) {
    setState(() {
      _movingBlock = null;
  ...
  Future<void> _saveDayToFirestore() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(translation(context: context, 'Saved'))));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    double? _editorTop;
    const double _editorHeight = 160.0;
    if (_editing != null) {
      final rawTop = _slotToDY(((_editing!.end.hour - _startHour) * 60 + _editing!.end.minute) ~/ _slotMinutes) + 6;
      final maxTop = totalSlots * _slotHeight - _editorHeight;
      _editorTop = math.min(rawTop, maxTop);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (_scroll.hasClients && _editorTop != null) {
          final desiredScroll = (_editorTop - 80).clamp(0.0, _scroll.position.maxScrollExtent);
        ...
        actions: [
          Padding(
            ...
            child: ToggleButtons(
              ...
              selectedBorderColor: theme.colorScheme.primary.withOpacity(0.9),
              borderColor: theme.colorScheme.onSurface.withOpacity(0.12),
              fillColor: theme.colorScheme.primary.withOpacity(0.14),
                                  ...
                                  child: isHour
                                      ? Padding(
                                          ...
                                          child: Text(
                                            ...
                                            style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurface.withOpacity(0.9)),
                                          ),
                                        )
                                ...
                                children: List.generate(totalSlots, (idx) {
                                  ...
                                  return Container(
                                    ...
                                    decoration: BoxDecoration(
                                      border: Border(top: BorderSide(color: theme.colorScheme.onSurface.withOpacity(isHour ? 0.32 : 0.12), width: isHour ? 1.0 : 0.6)),
                                    ),
                                  );
                                }),
                              ),
                                      ...
                                      child: Padding(
                                        ...
                                        child: GestureDetector(
                                          onTap: () {
                                            ...
                                            setState(() {
                                              _editing = b;
                                              _editingOriginalName = b.name;
                                              _editingWasNew = false;
                                              _editController.text = b.name;
                                            });
                                          },
                                          onPanStart: (details) => _onBlockPanStart(b, details),
                                          onPanUpdate: _onBlockPanUpdate,
                                          onPanEnd: _onBlockPanEnd,
                                          child: Container(
                                            decoration: BoxDecoration(
                                              color: theme.colorScheme.primary.withOpacity(0.18),
                                              borderRadius: BorderRadius.circular(10),
                                              border: Border.all(color: theme.colorScheme.primary.withOpacity(0.7)),
                                            ),
                              ...
                              if (_tempBlock != null)
                                Positioned(
                                  ...
                                  child: Opacity(
                                    opacity: 0.9,
                                    child: Container(
                                      decoration: BoxDecoration(color: theme.colorScheme.primary.withOpacity(0.22), borderRadius: BorderRadius.circular(8)),
                                    ),
                                  ),
                                ),
                                  ...
                                  child: Material(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text('${DateFormat('HH:mm').format(_editing!.start)} - ${DateFormat('HH:mm').format(_editing!.end)}', style: theme.textTheme.labelSmall),
                                          const SizedBox(height: 8),
                                          TextField(
                                          ...
                                          Row(
                                            ...
                                            children: [
                                              TextButton(onPressed: () async {
                                                // Close: apply changes locally (no Firestore) and close editor
                                                setState(() {
                                                  if (_editing != null) {
                                                  ...
                                                  _editingWasNew = false;
                                                  // update local draft and mark dirty
                                                  _localDrafts[_dayKeyForSelected()] = _blocks.map((b) => b.copyWith()).toList();
                                                  _dirtyDays.add(_dayKeyForSelected());
                                                });
                                              ...
                                              TextButton(
                                                onPressed: () async {
                                                  // Delete
                                                  if (_editing != null) {
                                                    _deleteBlock(_editing!);
                                                  }
                                                },
                                                child: Text(translation(context: context, 'Delete'), style: TextStyle(color: theme.colorScheme.error)),
                                              ),
                                            ],
                                          ),
                                        ],
